...
class DomNetwork:
    ...
    def getFullScores(self):
        ...
        #self.bestParamsAccuracy = np.load(name + '_params.npy')
...
if __name__ == '__main__':
    ...
    dataLength = len(pd.read_csv(args.file).reset_index(drop=True))
    errorArr = np.zeros((dataLength,), dtype=np.int32)
    arguments = []
    ...
    domModel = DomNetwork(trainDataset=dataFile, 
                          initialLag=0,
                          fullMatrix=False,
                          saveResults=False,
                          seed=0,
                          powellInitials=None,
                          verbose=False)
    flags, interactionCount, correctInteractionCount = domModel.getErrorMetrics(item[3])
    
    file_path = os.path.join(folderPath, name + '.txt')
    flags_str = np.array2string(flags)
    with open(file_path, 'w') as file:
        output_data = f"Found flags (interaction id from 0): {flags_str}\n"
